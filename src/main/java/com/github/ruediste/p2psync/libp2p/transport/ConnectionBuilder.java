...
import com.github.ruediste.p2psync.libp2p.transport.tcp.TcpConnection;

/**
 * Builds a fully upgraded {@link Connection} from a raw {@link Socket}. Runs the upgrade
 * sequence (security negotiation, then muxer negotiation) as plain sequential blocking calls.
 */
public final class ConnectionBuilder {

    private final ConnectionUpgrader upgrader;

    public ConnectionBuilder(ConnectionUpgrader upgrader) {
    ...
    public Connection upgrade(Socket socket, boolean isInitiator) {
        ...
        P2PStream stream = new P2PStream(in, out, isInitiator);

        Multiaddr localAddress = localAddress(socket);
        Multiaddr remoteAddress = remoteAddress(socket);
        PeerId remotePeerId = PeerId.random(); // placeholder until M5 extracts it from Noise

        TcpConnection connection = new TcpConnection(stream, localAddress, remoteAddress,
    ...
    private static Multiaddr remoteAddress(java.net.Socket socket) {
