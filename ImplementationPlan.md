...
│   ├── node/                       # Node configuration and management
...
- `generateNodeKeys()` (function): Creates a new Ed25519 keypair for node management.
...
- `NodeTrustSet` (class): Represents the authoritative NodeTrust configuration embedded inside `UserData`. Each entry has nodeId, reliability factor (0..1), and a vector clock. Merge strategy: per-nodeId last-writer-wins based on clock
...
- `NodeUserStorageQuota` (class): Stores the per-user storage limits. The maximum can be unlimited.
- `NodeConfiguration` (class): Encrypted and signed configuration for a node. Stored in `StorageUserData`. References one or more `NodeUserConfiguration`s. Created by a user who knows the node's management private key (stored in their `UserData`).
- `NodeUserConfiguration` (class): Contains storage quotas and user keys (subset of storage, link, data, or user keys) for a specific user. Defines the subset of data a node is authorized to replicate.

**Configuration Merging:** Multiple users can provide `NodeConfiguration`s for the same node. The node merges all available configurations by taking the union of all provided keys and the maximum storage quota per user ("most generous wins").

**Quota Application:** `ReplicationController` uses the merged configuration to enforce storage limits and determine which keys are available for replication and decryption.
...
- **NodeTrust:** Because NodeTrust entries live inside `UserData`, the same vector-clock merge applies. Concurrent edits to the same nodeId are resolved deterministically: higher clock wins; equal clocks fall back to lexicographic ordering of nodeId to keep merges stable. Deletes outweigh adds by setting a `removed` flag (or reliability=0) with a higher clock, ensuring that revoked trust propagates even if another node is offline. The entry is removed once the `UserData` of all nodes shows a higher clock than the one of the `NodeTrust`.
...
- `main()` (function): Orchestrates startup: (1) load/create user keys and node keys, (2) create libp2p/Helia node, (3) construct `InstanceComponents`, (4) construct and initialize `LifecycleComponents` in dependency order, (5) register protocol handler, (6) start async loops (DHT publish, DHT discover).

### 3.11 Frontend (`apps/frontend/`)
