...
- **`ManagedNodeEntry` DataObject:** (Storage layer) Each `UserData` carries a `managedNodes` map keyed by nodeId. The value is a `ManagedNodeEntry` data object with its own vector clock plus a set of per-user `QuotaPolicy` records (`userPublicKey → {maxBytes, maxShards, burstBytes}`) describing what that node is allowed to store for that replicated user. Entries are signed with the controlling user’s key (as part of the surrounding `StorageUserData`) so they travel through the normal replication pipeline.
...
| Current file                | Target location                                                                                        | Changes needed                                                                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| `src/index.ts`              | `src/index.ts`                                                                                         | Extract DHT loops into `DHTPublisher` / `DHTDiscovery`; use component system properly      |
| `src/components.ts`         | `src/components.ts`                                                                                    | Expand with new lifecycle components                                                       |
| `src/createNode.ts`         | `src/network/createNode.ts`                                                                            | Minimal changes; extract `MyHelper` to its own file if desired                             |
| `src/syncProtocol.ts`       | `src/network/syncProtocol.ts`                                                                          | Add new message types as proto definitions grow                                            |
| `src/UserNodeController.ts` | `src/network/UserNodeController.ts`, `UserPeerManager.ts`, `UserPeerEntry.ts`, `SyncMessageHandler.ts` | Split class; fix `this.store` → `this.components.dataStore` bug; complete message handlers |
| `src/UsersController.ts`    | `src/user/UserController.ts`                                                                           | Flesh out with actual user management logic                                                |
| `src/userKeysManagement.ts` | `src/user/UserKeysManagement.ts`                                                                       | Stable; minor path to `src/crypto/keys.ts` for key generation                              |
| `src/utils.ts`              | `src/util/sleep.ts`, `src/util/server.ts`                                                              | Split sleep from Express server                                                            |
| `src/mdns/`                 | `src/mdns/`                                                                                            | No changes needed                                                                          |
| `proto/*.proto`             | `proto/`                                                                                               | Add `storage.proto`, `link.proto`; expand `p2p-sync-protocol.proto` with full message set  |
