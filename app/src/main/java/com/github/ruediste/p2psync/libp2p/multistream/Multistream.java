...
/**
 * Given a fixed list of {@link ProtocolBinding}s, runs multistream-select over a {@link P2PStream}
 * (as either initiator or responder, according to {@link P2PStream#isInitiator()}), then invokes the
 * winning binding's {@link ProtocolBinding#init}.
 *
 * <p>
 * This exact combination is reused three times by later milestones: once per connection for
 * security negotiation (bindings = {@code [NoiseXXSecureChannel]}), once per connection for
 * muxer negotiation (bindings = {@code [YamuxStreamMuxer]}), and once per application stream
 * for protocol negotiation (bindings = the {@code Host}'s registered app protocols).
 *
 * <p>
 * Ported from {@code io.libp2p.multistream.MultistreamImpl} (jvm-libp2p); upstream's
 * {@code preHandler}/{@code postHandler}/pipeline-installation hooks and the
 * {@code CompletableFuture}-returning {@code initChannel} have no equivalent here — negotiation
 * and the binding's {@code init} are both plain blocking calls, so {@link #negotiate} simply
...
public final class Multistream<TController> {
     ...
     *
     * <p>
     * As initiator, announces every candidate protocol from every binding (in binding order,
     * each binding's own preference order). As responder, matches the initiator's proposals
    ...
    public Result<TController> negotiate(P2PStream stream) {
        ...
        ProtocolBinding<TController> binding = ProtocolSelect.select(bindings, selectedProtocol);
        TController controller = binding.init(stream, selectedProtocol);
        return new Result<>(selectedProtocol, controller);
    }

    /**
     * Negotiates as the initiator: proposes each of {@code protocols} in order until the
     * responder accepts one (by echoing it back) or all candidates are exhausted.
     *
     * @return the agreed protocol id.
     * @throws RuntimeException if none of {@code protocols} were accepted by the responder.
     */
    ...
    /**
     * Negotiates as the responder: waits for the initiator's proposals, accepting (by echoing
     * back) the first one that matches any of {@code matchers}, or replying {@code na} and
    ...
    /** The outcome of a successful negotiation: the agreed protocol id and its controller. */
