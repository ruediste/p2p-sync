 ...
 *
 * <p>
 * Ported from {@code io.libp2p.core.multiformats.Protocol} (jvm-libp2p),
 * trimmed to the
 * minimal subset needed here: {@code ip4}/{@code ip6}/{@code tcp}/{@code p2p}
 * (no DNS variants,
...
public enum Protocol {
    ...
    /**
     * protocol ids that carry a {@link PeerId} value (only {@code /p2p/} in this
     * port).
     */
    ...
    private static String addressBytesToString(byte[] bytes) {
        try {
            ...
            // Only thrown if bytes.length is neither 4 nor 16 - can't happen given our
            // callers.
