...
/**
 * Thin (un)marshaling helpers converting between {@link PubKey}/{@link PrivKey}
 * instances and
 * the generated
 * {@code com.github.ruediste.p2psync.proto.libp2p.Crypto.PublicKey}/{@code Crypto.PrivateKey}
 * protobuf messages.
 *
 * <p>
 * Ported from the marshaling functions in {@code io.libp2p.core.crypto.Key}
 * (jvm-libp2p);
 * trimmed to dispatch only to the Ed25519 implementation (see "Future extension
 * points" in
...
public final class Marshaling {
    ...
    public static byte[] marshalPrivateKey(PrivKey privKey) {
        return Libp2P.PrivateKey.newBuilder()
    ...
    public static PrivKey unmarshalPrivateKey(byte[] data) {
        Libp2P.PrivateKey proto;
        try {
            proto = Libp2P.PrivateKey.parseFrom(data);
